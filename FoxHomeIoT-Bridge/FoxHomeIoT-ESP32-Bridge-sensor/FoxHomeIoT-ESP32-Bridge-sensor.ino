...
//              message to mqtt + reading sensor data and send to mqtt
// © 2022 Jiří Kučera
...
const char* TOPIC_MQTT_SENSORS        = "bridge/sensors";
...
void loop() {
  ...
  static const unsigned long EVENT_INTERVAL_MS = 180000;
...
void ReadAndSendDataSensors () {
  ...
  if (client.publish(TOPIC_MQTT_SENSORS, payload.c_str())) {
    Serial.println("Published to " + String(TOPIC_MQTT_SENSORS) + ":");
    // Write JSON document to serial port
    serializeJson(root, Serial);
    Serial.println("");
  } else {
